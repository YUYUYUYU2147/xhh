...
export class tpl extends plugin {
  ...
  async tlp(e) {
    ...
    var max = kg.tlpcs;
    let num = await redis.get(`xhh:tlp:${e.user_id}`);
    if (num && num == max)
      return e.reply('你今日塔罗牌获取次数已达上限，明天再来吧', true);
    let url = 'http://api.tangdouz.com/tarot.php?&return=json';
    let data = await (await fetch(url)).json();
    if (!data.img) return logger.error('[小花火]塔罗牌接口出错');
    let img = data.img.replace(/\\/g, '');
    // await e.reply("开始洗牌.....",true, { recallMsg:  3 })
    await reply_recallMsg(e, '开始洗牌......', 3, true);
    await sleep(3010);
    await e.reply(
      [
        `${data.position == '逆位' ? '哎呀~' : '噔噔~'}，是〖${data.position}〗的塔罗牌：\n`,
        data.name,
        `(${data.namen})\n`,
        `基本牌意：${data.content}\n`,
        segment.image(img),
    ...
    if (!num) {
      await redis.set(`xhh:tlp:${e.user_id}`, 1, { EX: parseInt(new_date) });
    } else {
      num++;
      await redis.set(`xhh:tlp:${e.user_id}`, Number(num), {
